...
@Component({
  selector: 'app-usine-edit',
  templateUrl: './usine-edit.component.html',
  styleUrls: ['./usine-edit.component.scss'],
  providers: [
...
export class UsineEditComponent implements OnInit, OnDestroy {
   ...
   *
   * @param {Router} router
   * @param {UsineEditService} _usineEditService
   */
  constructor(private router: Router, private _usineEditService: UsineEditService,
    ...
    private dateAdapter: NgbDateAdapter<string>,private usineListService: UsineListService,
  ...
  submit(form: { valid: any; }) {
    if (form.valid) {
      this._usineEditService.create(this.currentRow)
      .subscribe(
        response => {
          this._toastrService.success('Mise à jour privileg avec success', '');
          this.router.navigate(['apps/gestion-dechets/usine/usine-list']);
        },
        error => {
   ...
   */
  ngOnInit(): void {
    this._usineEditService.onUsineEditChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(response => {
            this.rows = response;
            this.rows.map(row => {
