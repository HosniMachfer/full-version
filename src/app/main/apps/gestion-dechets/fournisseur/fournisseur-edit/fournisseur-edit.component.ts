...
export class FournisseurEditComponent implements OnInit, OnDestroy {
   ...
   *
   * @param {Router} router
   * @param {FournisseurEditService} _fournisseurEditService
   */
  constructor(private router: Router, private _fournisseurEditService: FournisseurEditService,
    ...
    private dateAdapter: NgbDateAdapter<string>,private fournisseurListService: FournisseurListService,
  ...
  submit(form: { valid: any; }) {
    if (form.valid) {
      this._fournisseurEditService.create(this.currentRow)
      .subscribe(
        response => {
          this._toastrService.success('Mise à jour privileg avec success', '');
          this.router.navigate(['apps/gestion-dechets/fournisseur/fournisseur-list']);
        },
        error => {
   ...
   */
  ngOnInit(): void {
    this._fournisseurEditService.onFournisseurEditChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(response => {
            this.rows = response;
            this.rows.map(row => {
