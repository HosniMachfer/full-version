...
@Component({
  selector: 'app-etat-magasin-edit',
  templateUrl: './etat-magasin-edit.component.html',
  styleUrls: ['./etat-magasin-edit.component.scss'],
  providers: [
...
export class EtatMagasinEditComponent implements OnInit, OnDestroy {
   ...
   *
   * @param {Router} router
   * @param {EtatMagasinEditService} _etatMagasinEditService
   */
  constructor(private router: Router, private _etatMagasinEditService: EtatMagasinEditService,
    ...
    private dateAdapter: NgbDateAdapter<string>,private etatMagasinListService: EtatMagasinListService,
  ...
  submit(form: { valid: any; }) {
    if (form.valid) {
      this._etatMagasinEditService.create(this.currentRow)
      .subscribe(
        response => {
          this._toastrService.success('Mise à jour privileg avec success', '');
          this.router.navigate(['apps/gestion-dechets/etat-magasin/etat-magasin-list']);
        },
        error => {
   ...
   */
  ngOnInit(): void {
    this._etatMagasinEditService.onEtatMagasinEditChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(response => {
            this.rows = response;
            this.rows.map(row => {
