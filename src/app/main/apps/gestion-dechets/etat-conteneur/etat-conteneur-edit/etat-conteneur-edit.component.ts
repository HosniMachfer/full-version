...
@Component({
  selector: 'app-etat-conteneur-edit',
  templateUrl: './etat-conteneur-edit.component.html',
  styleUrls: ['./etat-conteneur-edit.component.scss'],
  providers: [
...
export class EtatConteneurEditComponent implements OnInit, OnDestroy {
   ...
   *
   * @param {Router} router
   * @param {EtatConteneurEditService} _etatConteneurEditService
   */
  constructor(private router: Router, private _etatConteneurEditService: EtatConteneurEditService,
    ...
    private dateAdapter: NgbDateAdapter<string>,private etatConteneurListService: EtatConteneurListService,
  ...
  submit(form: { valid: any; }) {
    if (form.valid) {
      this._etatConteneurEditService.create(this.currentRow)
      .subscribe(
        response => {
          this._toastrService.success('Mise à jour privileg avec success', '');
          this.router.navigate(['apps/gestion-dechets/etat-conteneur/etat-conteneur-list']);
        },
        error => {
   ...
   */
  ngOnInit(): void {
    this._etatConteneurEditService.onEtatConteneurEditChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(response => {
            this.rows = response;
            this.rows.map(row => {
