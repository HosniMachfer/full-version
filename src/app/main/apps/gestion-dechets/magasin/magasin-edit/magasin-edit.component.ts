...
export class MagasinEditComponent implements OnInit, OnDestroy {
   ...
   *
   * @param {Router} router
   * @param {MagasinEditService} _magasinEditService
   */
  constructor(private router: Router, private _magasinEditService: MagasinEditService,
    ...
    private dateAdapter: NgbDateAdapter<string>,private magasinListService: MagasinListService,
  ...
  submit(form: { valid: any; }) {
    if (form.valid) {
      this._magasinEditService.create(this.currentRow)
      .subscribe(
        response => {
          this._toastrService.success('Mise à jour privileg avec success', '');
          this.router.navigate(['apps/gestion-dechets/magasin/magasin-list']);
        },
        error => {
   ...
   */
  ngOnInit(): void {
    this._magasinEditService.onMagasinEditChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(response => {
            this.rows = response;
            this.rows.map(row => {
