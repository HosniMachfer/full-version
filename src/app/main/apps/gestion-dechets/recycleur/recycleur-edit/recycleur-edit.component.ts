...
@Component({
  selector: 'app-recycleur-edit',
  templateUrl: './recycleur-edit.component.html',
  styleUrls: ['./recycleur-edit.component.scss'],
  providers: [
...
export class RecycleurEditComponent implements OnInit, OnDestroy {
   ...
   *
   * @param {Router} router
   * @param {RecycleurEditService} _recycleurEditService
   */
  constructor(private router: Router, private _recycleurEditService: RecycleurEditService,
    ...
    private dateAdapter: NgbDateAdapter<string>,private recycleurListService: RecycleurListService,
  ...
  submit(form: { valid: any; }) {
    if (form.valid) {
      this._recycleurEditService.create(this.currentRow)
      .subscribe(
        response => {
          this._toastrService.success('Mise à jour privileg avec success', '');
          this.router.navigate(['apps/gestion-dechets/recycleur/recycleur-list']);
        },
        error => {
   ...
   */
  ngOnInit(): void {
    this._recycleurEditService.onRecycleurEditChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(response => {
            this.rows = response;
            this.rows.map(row => {
