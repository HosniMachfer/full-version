...
export class MvtstockViewService implements Resolve<any> {
  ...
  getApiData(id: number): Promise<any[]> {
    return new Promise((resolve, reject) => {
      this._httpClient.get(`${environment.apiUrl}`+'/diva-erp-rest-api-gestion-dechets/mvtstock/'+`${id}`).subscribe((response: any) => {
        this.rows = response;
...
getMvtstock(route: ActivatedRouteSnapshot): Observable<any[]> {
    let currentId = Number(route.paramMap.get('id'));
    return this._httpClient.get<any>(`${environment.apiUrl}`+'/diva-erp-rest-api-gestion-dechets/mvtstock'+`${currentId}`,  httpOptions);
}
}
