...
@Component({
  selector: 'app-dechet-edit',
  templateUrl: './dechet-edit.component.html',
  styleUrls: ['./dechet-edit.component.scss'],
  providers: [
...
export class DechetEditComponent implements OnInit, OnDestroy {
   ...
   *
   * @param {Router} router
   * @param {DechetEditService} _dechetEditService
   */
  constructor(private router: Router, private _dechetEditService: DechetEditService,
    ...
    private dateAdapter: NgbDateAdapter<string>,private dechetListService: DechetListService,
  ...
  submit(form: { valid: any; }) {
    if (form.valid) {
      this._dechetEditService.create(this.currentRow)
      .subscribe(
        response => {
          this._toastrService.success('Mise à jour privileg avec success', '');
          this.router.navigate(['apps/gestion-dechets/dechet/dechet-list']);
        },
        error => {
   ...
   */
  ngOnInit(): void {
    this._dechetEditService.onDechetEditChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(response => {
            this.rows = response;
            this.rows.map(row => {
