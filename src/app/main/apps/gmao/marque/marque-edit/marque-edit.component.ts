...
@Component({
  ...
  styleUrls: ['./marque-edit.component.scss'],
  providers: [
...
export class MarqueEditComponent implements OnInit, OnDestroy {
   ...
   *
   * @param {Router} router
   * @param {MarqueEditService} _MarqueEditService
   */
  constructor(private router: Router, private _marqueEditService: MarqueEditService,
    ...
    private dateAdapter: NgbDateAdapter<string>,private marqueListService: MarqueListService,
  ...
  submit(form: { valid: any; }) {
    if (form.valid) {
      this._marqueEditService.create(this.currentRow)
      .subscribe(
        response => {
   ...
   */
  ngOnInit(): void {
    this._marqueEditService.onMarqueEditChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(response => {
            this.rows = response;
            this.rows.map(row => {
