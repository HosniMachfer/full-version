...
@Component({
  selector: 'app-etatActuel-edit',
  templateUrl: './etat-actuel-edit.component.html',
  styleUrls: ['./etat-actuel-edit.component.scss'],
  providers: [
...
export class EtatActuelEditComponent implements OnInit, OnDestroy {
   ...
   *
   * @param {Router} router
   * @param {EtatActuelEditService} _EtatActuelEditService
   */
  constructor(private router: Router, private _etatActuelEditService: EtatActuelEditService,
    ...
    private dateAdapter: NgbDateAdapter<string>,private etatActuelListService: EtatActuelListService,
  ...
  submit(form: { valid: any; }) {
    if (form.valid) {
      this._etatActuelEditService.create(this.currentRow)
      .subscribe(
        response => {
          this._toastrService.success('Mise � jour privileg avec success', '');
          this.router.navigate(['apps/gmao/etatActuel/etatActuel-list']);
        },
        error => {
   ...
   */
  ngOnInit(): void {
    this._etatActuelEditService.onEtatActuelEditChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(response => {
            this.rows = response;
            this.rows.map(row => {
