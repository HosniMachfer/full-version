...
@Component({
  selector: 'app-localite-edit',
  templateUrl: './localite-edit.component.html',
  styleUrls: ['./localite-edit.component.scss'],
  providers: [
...
export class LocaliteEditComponent implements OnInit, OnDestroy {
   ...
   *
   * @param {Router} router
   * @param {LocaliteEditService} _LocaliteEditService
   */
  constructor(private router: Router, private _localiteEditService: LocaliteEditService,
    ...
    private dateAdapter: NgbDateAdapter<string>,private localiteListService: LocaliteListService,
  ...
  submit(form: { valid: any; }) {
    if (form.valid) {
      this._localiteEditService.create(this.currentRow)
      .subscribe(
        response => {
   ...
   */
  ngOnInit(): void {
    this._localiteEditService.onLocaliteEditChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(response => {
            this.rows = response;
            this.rows.map(row => {
