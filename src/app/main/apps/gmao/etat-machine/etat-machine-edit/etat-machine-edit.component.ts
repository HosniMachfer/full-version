...
@Component({
  selector: 'app-etat-machine-edit',
  templateUrl: './etat-machine-edit.component.html',
  styleUrls: ['./etat-machine-edit.component.scss'],
  providers: [
...
export class EtatMachineEditComponent implements OnInit, OnDestroy {
   ...
   *
   * @param {Router} router
   * @param {EtatMachineEditService} _EtatMachineEditService
   */
  constructor(private router: Router, private _etatMachineEditService: EtatMachineEditService,
    ...
    private dateAdapter: NgbDateAdapter<string>,private etatMachineListService: EtatMachineListService,
  ...
  submit(form: { valid: any; }) {
    if (form.valid) {
      this._etatMachineEditService.create(this.currentRow)
      .subscribe(
        response => {
          this._toastrService.success('Mise � jour privileg avec success', '');
          this.router.navigate(['apps/gmao/etat-machine/etat-machine-list']);
        },
        error => {
   ...
   */
  ngOnInit(): void {
    this._etatMachineEditService.onEtatMachineEditChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(response => {
            this.rows = response;
            this.rows.map(row => {
