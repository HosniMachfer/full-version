...
@Component({
  selector: 'app-temps-fct',
  templateUrl: './temps-fct.component.html',
  styleUrls: ['./temps-fct.component.scss'],
  encapsulation:ViewEncapsulation.Emulated
})
export class TempsFctComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<any>(this.Url).subscribe(data => {
      ...
      for(let i in data ){
        this.obj={
       name : data[i].machine,
       value : ((data[i].dureetRV -data[i].dureePanne)/60)
        }
