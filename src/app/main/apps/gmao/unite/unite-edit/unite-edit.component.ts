...
export class UniteGmaoEditComponent implements OnInit, OnDestroy {
   ...
   *
   * @param {Router} router
   * @param {UniteEditService} _uniteEditService
   */
  constructor(private router: Router, private _uniteEditService: UniteGmaoEditService,
    ...
    private dateAdapter: NgbDateAdapter<string>,private uniteListService: UniteGmaoListService,
  ...
  submit(form: { valid: any; }) {
    if (form.valid) {
      this._uniteEditService.create(this.currentRow)
      .subscribe(
        response => {
          this._toastrService.success('Mise � jour privileg avec success', '');
          this.router.navigate(['apps/gmao/unite/unite-list']);
        },
        error => {
   ...
   */
  ngOnInit(): void {
    this._uniteEditService.onUniteEditChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(response => {
            this.rows = response;
            this.rows.map(row => {
