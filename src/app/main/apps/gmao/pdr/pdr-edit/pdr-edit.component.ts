...
@Component({
  selector: 'app-pdr-edit',
  templateUrl: './pdr-edit.component.html',
  styleUrls: ['./pdr-edit.component.scss'],
  providers: [
...
export class PDREditComponent implements OnInit, OnDestroy {
   ...
   *
   * @param {Router} router
   * @param {PDREditService} _pdrEditService
   */
  constructor(private router: Router, private _pdrEditService: PDREditService,
  ...
  submit(form: { valid: any; }) {
    if (form.valid) {
      this._pdrEditService.create(this.currentRow)
      .subscribe(
        response => {
          this._toastrService.success('Mise � jour privileg avec success', '');
          this.router.navigate(['apps/gmao/pdr/pdr-list']);
        },
        error => {
   ...
   */
  ngOnInit(): void {
    this._pdrEditService.onPDREditChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(response => {
            this.rows = response;
            this.rows.map(row => {
